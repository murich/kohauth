...
class Hybrid_Providers_Twitter extends Hybrid_Provider_Model_OAuth1
{
	...
	*/
	function getUserProfile()
	{
		...
		if ( $this->api->http_code != 200 ){
			throw new Exception( "User profile request failed! {$this->providerId} returned an error. " . $this->errorMessageByStatus( $this->api->http_code ).'<br> '.print_r($response, 1), 6 );
		}
	...
	*/ 
	function getUserActivity( $stream )
	{
		...
		elseif($stream == "timeline"){
			$response  = $this->api->get( 'statuses/home_timeline.json' ); 
		}
        else{
            $response  = $this->api->get( 'statuses/user_timeline.json?screen_name='.$stream );
        }
